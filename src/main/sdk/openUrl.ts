...
import { debug, error, info } from '@dev'
...
const parseLink = (link: string | undefined): InvokedLink | null => {
  info('[InvokedLink] Parsing:', link)
  if (!link) return null
  const parsedUrl = new URL(link)
  const hostname = parsedUrl.hostname
  const query = parsedUrl.searchParams.get('q')
  debug('[InvokedLink] Action:', hostname)
  debug('[InvokedLink] Query:', query)
...
export const setupOpenUrl = (onLinking?: (str: InvokedLink) => void) => {
  if (!setProtocol()) {
    error(`Failed to set APP_PROTOCOL=${APP_PROTOCOL}`)
    return
  }

  const gotTheLock = app.requestSingleInstanceLock()
  if (!gotTheLock) {
    info('An instance is already running. Closing...')
    app.quit()
  } else {
    const handleProtocolArgs = (argv: string[]) => {
      info('Second instance stopped', argv)
