...
import { RecommendationInfo } from '@shared/types'
...
const fetchRecommendations = asyncAction(
  ...
  async (animeName: string, api) => {
    ...
    if (Array.isArray(recs)) {
      ...
      api.dispatch(recommendations.setStatus({ name: animeName, status: 'succeeded' }))
    } else {
...
export const slice = createSlice({
  ...
  reducers: {
    ...
    setRecommendations(
      state,
      { payload }: PayloadAction<{ name: string; recommendations: RecommendationInfo[] }>
    ) {
      if (!state[payload.name]) {
        state[payload.name] = {}
      }
