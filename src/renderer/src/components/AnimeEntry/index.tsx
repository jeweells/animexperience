...
const createMediaQueriesUntil = (
  ...
  const queries: ReturnType<typeof css>[] = []

  for (let n = 1; ; n++) {
    const mediaWidth = targetImageSize.width * n + (n - 1) * props.gap + 2 * props.navigationWidth
...
export const AnimeEntry = React.memo<AnimeEntryProps>(
  ({ onClick, onMouseOver, onMouseOut, isPopover, index, visible, sliding, render }) => {
