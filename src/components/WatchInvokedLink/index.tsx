...
export type WatchInvokedLinkProps = {}

export const WatchInvokedLink: React.FC<WatchInvokedLinkProps> =
    React.memo<WatchInvokedLinkProps>(({}) => {
        const status = useAppSelector((d) => d.invokedLink.status.watch)
        const { data: info, request } =
            useAppSelector((d) => d.invokedLink.preAllow.watch) ?? {}
        ...
        return (
            <>
                ...
                {ready ? (
                    <UnhookedAnimePeek
                        info={info}
                        contentProps={{ style: { paddingTop: 52 + 32 } }}
                    />
                ) : (
                    <AnimePeekPlaceholder
                        contentProps={{ style: { paddingTop: 52 + 32 } }}
                    />
                )}
                <Actions>
