...
import { FollowedAnime } from '~/globals/types'
...
describe('FollowedAnimesUpdates', () => {
    ...
    beforeEach(() => {
        ...
        initialState = {
            ...
            followedAnimes: {
                ...
                followed,
                followedDict: (followed as FollowedAnime[]).reduce((acc, curr) => {
                    acc[curr.name] = curr
                    return acc
    ...
    it('renders when all animes are loading', async () => {
        initialState.followedAnimes!.followed!.forEach(
            (anime) => (anime!.status = 'loading'),
        )
        const wrapper = render(getComponent())
        expect(wrapper.baseElement).toMatchSnapshot()
    })
