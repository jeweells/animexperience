...
export const getJKAnimeEpisodeVideos = async (anime: string, episode: number) => {
    ...
    if (Array.isArray(results) && results.length > 0) {
        ...
        const scripts = $("script:not([src])").map((idx, elm) => $(elm).html()).toArray();
...
export const getAnimeIDEpisodeVideos = async (episodeLink: string) => {
    ...
    const datas = $("#partes .subtab .parte").map((elm, b) => {
    ...
    }).toArray().filter((x): x is { name: string; data: string; } => !!x.data)
        .map(x => {
